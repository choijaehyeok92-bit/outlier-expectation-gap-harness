...
- **`.env`를 읽는다.** 웹 `/settings` 화면에서 직접 넣는 것이 가장 쉽다 — 저장하면
  **재시작 없이 즉시 반영**된다. 이 화면이 이 서비스에서 **비밀값을 받는 유일한 라우트**이고,
  경계를 좁게 그어 뒀다:

  - **쓰기 전용.** 어떤 라우트도 값을 돌려주지 않으므로 화면은 저장된 값을 보여줄 수 없다.
    입력칸은 항상 비어 있고, 저장은 수정이 아니라 교체다.
  - **로컬 호출만 허용.** 요청이 loopback이 아니면 403이고, 화면이 그 이유를 표시한다.
    API를 `0.0.0.0`에 띄웠다면 키를 HTTP로 보내지 말고 그 기계의 `.env`를 직접 고친다.
  - **완전히 끌 수 있다.** `HARNESS_DISABLE_KEY_WRITES=1`이면 저장이 거부된다.
  ...
  **명령줄에 키를 적지 않는다.** PSReadLine이 입력한 명령을 평문 파일
  ...
  `HARNESS_ENV_FILE`로 체크아웃 밖의 경로를 가리킬 수도 있다.
