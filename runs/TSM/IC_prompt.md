# 과제: TSM / 기준일 2026-09-18 / investment_committee (IC)
저장소: /home/runner/work/outlier-expectation-gap-harness/outlier-expectation-gap-harness. 작성할 파일: runs/TSM/reports/IC.json, runs/TSM/final_verdict.json, runs/TSM/one_page_investment_record.md. 그 외 파일은 수정하지 않는다.
...
## 입력
runs/TSM/digest.md와 runs/TSM/aggregate.json (없으면 `python harness.py aggregate TSM` 후 `digest TSM` 실행). 다른 에이전트의 원 보고서는 특정 주장을 검증할 때만 해당 파일 하나를 연다.
...
작성 후 `python harness.py validate TSM IC`로 검증한다.
