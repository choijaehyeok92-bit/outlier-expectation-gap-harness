# 과제: MSFT / 기준일 2026-09-17 / evidence_quality (ED)
저장소: /home/runner/work/outlier-expectation-gap-harness/outlier-expectation-gap-harness. 작성할 파일: runs/MSFT/reports/ED.json. 그 외 파일은 수정하지 않는다.
...
## 입력
runs/MSFT/digest.md (없으면 `python harness.py aggregate MSFT` 후 `digest MSFT` 실행). 다른 에이전트의 원 보고서는 특정 주장을 검증할 때만 해당 파일 하나를 연다.
...
작성 후 `python harness.py validate MSFT ED`로 검증한다.
