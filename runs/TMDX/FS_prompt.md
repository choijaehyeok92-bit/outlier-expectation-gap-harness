# 과제: TMDX / 기준일 2026-09-18 / financial_survival (FS)
저장소: /home/runner/work/outlier-expectation-gap-harness/outlier-expectation-gap-harness. 작성할 파일: runs/TMDX/reports/FS.json. 그 외 파일은 수정하지 않는다.
    ...
    "criteria": [
      ...
      },
      {
        ...
        "observable_anchors": {
          ...
          "interpolation": {
            "mode": "none",
            "reason": "두 축의 min이며 공표된 행이 서로 겹친다(\"변화 없음 또는 감소\" vs \"감소\"). 정책 의미를 바꾸지 않고는 수치 경계를 유도할 수 없어 보간하지 않는다. 행 정의 정리가 선행되어야 한다."
          }
        }
      }
    ]
...
작성 후 `python harness.py validate TMDX FS`로 검증한다.
