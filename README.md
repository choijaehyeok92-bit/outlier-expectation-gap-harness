...
**대응 4 — Veto 구성요건 정의(`veto_criteria`).** 점수뿐 아니라 Hard Veto 층에서도 프로바이더가 갈렸다.
같은 NVDA 사실관계에서 Opus 5는 3건을 `conditional`로, gpt-5.6-sol은 전부 `cleared`로 판정했다.
원인은 veto를 "중대한 리스크"로 읽느냐 "이 문장이 실제로 성립하는가"로 읽느냐의 차이였다.

`veto_criteria`가 9개 문구마다 **구성요건(elements)·해소조건(cleared_if)·관할 밖(not_covered)**을 명시한다.
구성요건 중 하나라도 증거로 반증되면 `cleared`이며, 우려는 점수·`uncertainties`·모니터링으로 보낸다.
`anchor_policy`의 중복 감점 금지를 veto 층까지 확장해, 도메인 점수에 이미 온전히 반영된 사실만으로는
veto를 세우지 못하게 했다. 담당 에이전트의 프롬프트에 해당 veto의 정의가 자동으로 실린다.

이 정의를 적용하자 NVDA의 미해소 3건이 모두 `cleared`가 됐다.
- "고객가치 없이 마케팅·보조금에 의존하는 성장" — 필수 요건인 **고객가치 부재**가 gross margin 75.0%,
  판관비/매출 1.4%, 고객 선수금 $15.6B으로 반증된다.
- "단일 제품·단일 고객·단일 규제에 대한 **치명적** 종속성" — 치명성 요건이 실전 스트레스로 반증된다.
  중국 데이터센터 시장에서 사실상 배제되고도 Q2 매출은 전년 대비 +105.9%였다.
- "파산 또는 영구손실 확률이 기대수익에 비해 지나치게 높음" — 영구손실을 **사업가치의 영구적 소멸**로
  정의하면 순현금과 Bear 시나리오 FCF 창출로 반증된다. 매수가 대비 가격 하락은 EV·AS 점수가 이미 반영하므로
  여기서 다시 세우면 중복이다.

**관할 밖으로 라우팅한 우려 — 벤더 금융.** 위 재판정에서 기존 9개 문구 어디에도 "공급자가 고객의 구매자금을
지분투자·대출·보증으로 대는 구조"의 구성요건이 없음이 드러났다. 신설을 검토했으나 **채택하지 않았다** —
판정 기준인 대상 기업향 매출 비중이 대부분의 기업에서 공시되지 않아 실무상 영구 `conditional`로 남고
게이트가 상시 `UNRESOLVED`가 되기 때문이다. 측정할 수 없는 조건을 게이트로 두면 veto 층 전체의 신뢰도가 떨어진다.

대신 `veto_criteria.out_of_scope_concerns`에 결정과 라우팅을 기록했다. 위험은 CP `unit_economics`,
FS `dilution_offbalance`, AS `permanent_loss` 점수와 `key_kpis`(DSO, 지분증권 매입 + 보증 notional)로
추적하며, 대상 기업향 매출이 총매출의 25%를 넘는 것이 확인되면 "고객가치 없이 마케팅·보조금에 의존하는 성장"의
두 번째 요건이 충족되므로 그 veto로 판정한다.

**대응 5 — `harness.py calibrate`.** 두 실행의 subscore를 criterion 단위로 대조해 격차를 수치로 낸다.
